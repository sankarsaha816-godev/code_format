...
func TestHSBCParser_SplitLineJoinWithoutTabs(t *testing.T) {
	p := &HSBCParser{}
	...
	if strings.Contains(info.Transactions[0].Description, "TO 29JAN2026") {
		t.Errorf("BALANCE BROUGHT FORWARD should not include continuation text: %q", info.Transactions[0].Description)
	}

	middle := info.Transactions[1]
	if !strings.Contains(strings.ToUpper(middle.Description), "CR GROSS INTEREST TO 29JAN2026") {
		t.Errorf("unexpected middle transaction description: %q", middle.Description)
	}
	if middle.Amount != 6.07 {
		t.Errorf("middle transaction amount: got %.2f, want 6.07", middle.Amount)
	}
	if middle.Balance != 5113.94 {
		t.Errorf("middle transaction balance: got %.2f, want 5113.94", middle.Balance)
	}
}
